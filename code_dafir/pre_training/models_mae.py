...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size**2 * (in_chans-2), bias=True) # decoder to patch 最后只有一个通道。
        # --------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        inputs1 = torch.zeros(2, 3, 256, 256).cuda()
        inputs2 = torch.zeros(2, 1, 256, 256).cuda()  

        outs = net(inputs1,inputs2)
